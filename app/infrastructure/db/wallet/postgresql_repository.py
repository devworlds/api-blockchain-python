...
# Monitoring imports
from app.shared.monitoring.logging import get_logger, LoggerMixin, log_vault_operation, log_database_operation
...
class HashiCorpVaultService(VaultService, LoggerMixin):
    ...
    def store_private_key(self, key_id: str, private_key: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"Private key stored successfully - Key ID: {key_id}, Duration: {duration:.3f}s")
            record_vault_operation("store_private_key", "success", duration)
            
        except Exception as e:
            duration = time.time() - start_time
            self.logger.error(f"Failed to store private key - Key ID: {key_id}, Error: {str(e)}, Duration: {duration:.3f}s")
    ...
    def get_private_key(self, key_id: str) -> str:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.info(f"Private key retrieved successfully - Key ID: {key_id}, Duration: {duration:.3f}s")
            ...
            if "InvalidPath" in error_message or "404" in error_message:
                self.logger.error(f"Private key not found in Vault - Key ID: {key_id}. The wallet may need to be recreated. Error: {error_message}, Duration: {duration:.3f}s")
                record_vault_operation("get_private_key", "not_found", duration)
                raise ValueError(f"Private key not found in Vault for wallet {key_id.replace('eth_wallet_', '')}. This wallet may need to be recreated.")
            else:
                self.logger.error(f"Failed to retrieve private key - Key ID: {key_id}, Error: {error_message}, Duration: {duration:.3f}s")
...
class EthereumWalletService(WalletService, LoggerMixin):
    def create_wallet(self) -> Dict[str, str]:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.info(f"Ethereum wallet created successfully - Address: {wallet_data['address']}, Duration: {duration:.3f}s")
            ...
            duration = time.time() - start_time
            self.logger.error(f"Failed to create Ethereum wallet - Error: {str(e)}, Duration: {duration:.3f}s")
    ...
    def sign_transaction(self, private_key: str, transaction: dict) -> str:
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.info(f"Transaction signed successfully - Signer: {acct.address}, Duration: {duration:.3f}s")
            record_wallet_operation("sign_transaction", "success")
            return signed_tx
        except Exception as e:
            duration = time.time() - start_time
            self.logger.error(f"Failed to sign transaction - Error: {str(e)}, Duration: {duration:.3f}s")
...
class PostgreSQLWalletRepository(WalletRepository, LoggerMixin):
            ...
            with MetricsContext("save_wallet", "database"):
                async with self._pool.acquire() as conn:
                    await conn.execute(
                        ...
                        wallet.address, wallet.created_at, wallet.updated_at, wallet.deleted_at
                    )
                    
            duration = time.time() - start_time
            self.logger.info(f"Wallet saved successfully - Address: {wallet.address}, Duration: {duration:.3f}s")
            record_database_operation("save_wallet", "wallets", "success", duration)
            
        except Exception as e:
            duration = time.time() - start_time
            self.logger.error(f"Failed to save wallet - Address: {wallet.address}, Error: {str(e)}, Duration: {duration:.3f}s")
    ...
    async def get_wallet_by_address(self, address: str) -> Wallet | None:
        ...
        try:
            ...
            self.logger.info(f"Wallet fetch completed - Address: {address}, Found: {found}, Duration: {duration:.3f}s")
            record_database_operation("get_wallet_by_address", "wallets", "success", duration)
            
            return wallet
            
        except Exception as e:
            duration = time.time() - start_time
            self.logger.error(f"Failed to fetch wallet - Address: {address}, Error: {str(e)}, Duration: {duration:.3f}s")
            record_database_operation("get_wallet_by_address", "wallets", "error", duration)
            raise
    ...
    async def list_wallets(self) -> list[Wallet]:
        ...
        try:
            with MetricsContext("list_wallets", "database"):
                async with self._pool.acquire() as conn:
                    rows = await conn.fetch('SELECT address, created_at, updated_at, deleted_at FROM wallets')
                    wallets = [
                        Wallet(
                            address=row['address'],
                            created_at=row['created_at'],
            ...
            duration = time.time() - start_time
            count = len(wallets)
            self.logger.info(f"Wallets listed successfully - Count: {count}, Duration: {duration:.3f}s")
            ...
            duration = time.time() - start_time
            self.logger.error(f"Failed to list wallets - Error: {str(e)}, Duration: {duration:.3f}s")
