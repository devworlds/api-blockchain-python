...
transaction_value_total = Counter(
    "transaction_value_total", "Total value of transactions processed", ["asset"]
)

# Blockchain Metrics
blockchain_confirmations = Histogram(
    "blockchain_confirmations", "Number of confirmations for transactions", ["asset"]
)
...
wallets_created_total = Counter(
    "wallets_created_total", "Total number of wallets created"
)

wallet_operations_total = Counter(
    "wallet_operations_total", "Total wallet operations", ["operation", "status"]
)

# Vault Metrics
vault_operations_total = Counter(
    "vault_operations_total", "Total Vault operations", ["operation", "status"]
)

vault_operation_duration_seconds = Histogram(
    "vault_operation_duration_seconds", "Vault operation duration", ["operation"]
)
...
database_connection_pool_size = Gauge(
    "database_connection_pool_size", "Current database connection pool size"
)

database_connection_pool_used = Gauge(
    "database_connection_pool_used", "Current database connection pool used connections"
)

database_connection_pool_idle = Gauge(
    "database_connection_pool_idle", "Current database connection pool idle connections"
)
...
def record_blockchain_operation(operation: str, status: str, duration: float = None):
    ...
    if duration is not None:
        blockchain_operation_duration_seconds.labels(operation=operation).observe(
            duration
        )
