...
class TransactionMonitorService(LoggerMixin):
    ...
    async def start(self):
        ...
        self.logger.info(f"Transaction monitor service started - Min confirmations: {self.min_confirmations}, Poll interval: {self.poll_interval}s")
    ...
    async def _check_pending_transactions(self):
        ...
        try:
            ...
            self.logger.info(f"Checking {len(pending_transactions)} pending transactions")
    ...
    async def _check_transaction_status(self, tx_hash: str):
        ...
        try:
            ...
            if confirmations >= self.min_confirmations:
                ...
                success = await self.db_repo.update_transaction_status(tx_hash, "confirmed")
                if success:
                    self.logger.info(f"Transaction {tx_hash[:10]}... confirmed and updated ({confirmations} confirmations)")
                else:
                    self.logger.warning(f"Failed to update transaction status for {tx_hash[:10]}...")
            else:
                self.logger.debug(f"Transaction {tx_hash[:10]}... still pending ({confirmations}/{self.min_confirmations} confirmations)")
                
        except Exception as e:
            self.logger.error(f"Error checking transaction status for {tx_hash[:10]}...: {str(e)}")
...
class TransactionMonitorManager:
    ...
    async def health_check(self) -> dict:
        ...
        return {
            ...
            "status": "healthy" if all(m.running for m in self.monitors) else "degraded"
        } 