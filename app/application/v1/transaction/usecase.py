...
import re
from app.application.v1.transaction.schemas import TransactionOnChainRequest, TransactionOnChainResponse
from app.application.v1.transaction.dto import TransactionDTO
from app.infrastructure.blockchain.transaction.node_repository import Web3TransactionRepository
from app.infrastructure.db.transaction.postgresql_repository import PostgreSQLTransactionRepository
from app.infrastructure.db.wallet.postgresql_repository import PostgreSQLWalletRepository
import datetime
...
# Monitoring imports
from app.shared.monitoring.logging import get_logger, LoggerMixin, log_function_call, log_blockchain_operation, log_database_operation
...
class GetTransactionHash(LoggerMixin):
    def __init__(self, tx_hash: str, web3_repo: Web3TransactionRepository, wallet_repo: PostgreSQLWalletRepository, db_repo: PostgreSQLTransactionRepository, min_confirmations: int = 12):
    ...
    @track_time(transaction_processing_duration_seconds, {"operation": "validate_transaction", "asset": "unknown"})
    async def execute(self, txhash: str):
        self.logger.info(f"Starting transaction validation - Hash: {self.tx_hash}, Min confirmations: {self.min_confirmations}")
        ...
        try:
            ...
            if is_to_our_wallet and not is_from_our_wallet:
                ...
                self.logger.info(f"External deposit detected (from external to our wallet) - Type: {transaction_type}")
            elif is_from_our_wallet and not is_to_our_wallet:
                # Transaction from our wallet to external - this is a withdrawal
                transaction_type = "withdraw"
                self.logger.info(f"External withdrawal detected (from our wallet to external) - Type: {transaction_type}")
                ...
                transaction_type = "withdraw"
                self.logger.info(f"Internal transfer detected (both addresses are ours) - Type: {transaction_type}")
            else:
                # Neither address is ours - this shouldn't happen in this context
                transaction_type = "unknown"
                self.logger.warning(f"Neither address belongs to our wallets - this shouldn't happen here")
            
            # Save transaction if either address is ours
            if should_save_transaction:
                formatted_tx_hash = self.tx_hash if self.tx_hash.startswith('0x') else f'0x{self.tx_hash}'
                ...
                if not existing:
                    ...
                    self.logger.info(f"Transaction already exists in database. Skipping save - Hash: {formatted_tx_hash}, Status: {existing.status}")
            ...
            self.logger.info(f"Transaction validation completed successfully - Hash: {self.tx_hash}, Token: {is_token}, Confirmations: {confirmations}, Confirmed: {is_confirmed}, Our wallet: {is_our_wallet}, Duration: {duration:.3f}s")
            ...
            duration = time.time() - start_time
            self.logger.error(f"Transaction validation failed - Hash: {self.tx_hash}, Error: {str(e)}, Duration: {duration:.3f}s")
            record_blockchain_operation("validate_transaction", "error")
            raise

class CreateOnChainTransaction(LoggerMixin):
    def __init__(self, web3_repo: Web3TransactionRepository, db_repo: PostgreSQLTransactionRepository, vault_service, wallet_service):
        self.web3_repo = web3_repo
        self.db_repo = db_repo
        self.vault_service = vault_service
        self.wallet_service = wallet_service

    @track_time(transaction_processing_duration_seconds, {"operation": "create_transaction", "asset": "unknown"})
    async def execute(self, request: TransactionOnChainRequest) -> TransactionOnChainResponse:
        value_in_wei = request.get_value_in_wei()
        
        self.logger.info(f"Starting transaction creation - From: {request.address_from}, To: {request.address_to}, Asset: {request.asset}, Value: {request.value} ETH ({value_in_wei} Wei)")
        
        start_time = time.time()
        
        try:
            # Validate required fields
            if not request.address_from or not request.address_to or not request.asset or not request.value:
                self.logger.error(f"Missing required fields - From: {bool(request.address_from)}, To: {bool(request.address_to)}, Asset: {bool(request.asset)}, Value: {bool(request.value)}")
            ...
            with MetricsContext("build_transaction", "blockchain"):
                ...
                self.logger.info(f"Address balance check - Address: {address_from}, ETH Balance: {eth_balance / 1e18:.6f} ETH")
                
                if eth_balance == 0:
                    self.logger.error(f"Insufficient ETH balance for gas fees - Address: {address_from}, Balance: 0 ETH")
                    raise HTTPException(HTTPStatus.BAD_REQUEST, f"Insufficient ETH balance for gas fees. Address {address_from} has 0 ETH balance.")
                ...
                if request.asset.lower() == 'eth':
                    ...
                    if eth_balance < total_required:
                        ...
                        self.logger.error(f"Insufficient ETH balance - Required: {required_eth:.6f} ETH (value + gas), Available: {available_eth:.6f} ETH")
                    ...
                    self.logger.info(f"ETH transaction built - Value: {value_in_wei / 1e18:.6f} ETH, Max fee: {max_fee_per_gas}, Priority fee: {max_priority_fee}, Nonce: {nonce}, Gas cost: {estimated_gas_cost / 1e18:.6f} ETH")
                else:
                    if not request.contract_address:
                        self.logger.error(f"Contract address required for token transaction - Asset: {request.asset}")
                        raise HTTPException(HTTPStatus.BAD_REQUEST, "contract_address is required for tokens")
                    ...
                    contract = web3.eth.contract(address=Web3.to_checksum_address(request.contract_address), abi=erc20_abi)
                    data = contract.functions.transfer(address_to, value_in_wei).build_transaction({'gas': 0, 'gasPrice': 0})['data']
                    ...
                    if eth_balance < estimated_gas_cost:
                        ...
                        available_eth = eth_balance / 1e18
                        self.logger.error(f"Insufficient ETH balance for token transaction gas - Required: {required_eth:.6f} ETH, Available: {available_eth:.6f} ETH")
                        raise HTTPException(HTTPStatus.BAD_REQUEST, 
                                          f"Insufficient ETH balance for gas fees. Required: {required_eth:.6f} ETH, Available: {available_eth:.6f} ETH")
                    
                    contract_address = request.contract_address
                    self.logger.info(f"Token transaction built - Asset: {request.asset}, Contract: {contract_address}, Max fee: {max_fee_per_gas}, Priority fee: {max_priority_fee}, Nonce: {nonce}, Gas: {tx['gas']}, Gas cost: {estimated_gas_cost / 1e18:.6f} ETH")

            # 2. Get private key from Vault and sign transaction
            self.logger.info(f"Signing transaction with Vault - Address: {address_from}")
            
            # Add detailed logging for debugging
            self.logger.info(f"Transaction details before signing - Chain ID: {chain_id}, Nonce: {nonce}, Gas: {tx['gas']}, Max fee: {max_fee_per_gas}, Priority fee: {max_priority_fee}")
            ...
            with MetricsContext("sign_transaction", "vault"):
                ...
                self.logger.info(f"Transaction signed successfully - Length: {len(signed_tx)}, Prefix: {signed_tx[:20]}")
            ...
            with MetricsContext("broadcast_transaction", "blockchain"):
                # Log before sending
                self.logger.info(f"Sending raw transaction to network - Length: {len(signed_tx)}, Chain ID: {web3.eth.chain_id}")
            ...
            with MetricsContext("save_transaction", "database"):
                ...
                await self.db_repo.save_transaction(
                    TransactionEntity(
                        ...
                        updated_at=now,
                        deleted_at=None,
                        contract_address=request.contract_address if request.contract_address is not None else None
                    )
                )
            ...
            duration = time.time() - start_time
            self.logger.info(f"Transaction created successfully - Hash: {formatted_tx_hash}, Asset: {request.asset}, Status: {status}, Fee: {float(wei_to_eth(effective_fee)):.6f} ETH, Duration: {duration:.2f}s")
            ...
            if "Private key not found in Vault" in str(e):
                duration = time.time() - start_time
                self.logger.error(f"Transaction creation failed - Wallet key not found - From: {request.address_from}, To: {request.address_to}, Asset: {request.asset}, Error: {str(e)}, Duration: {duration}s")
                record_transaction_created(request.asset, "error")
                raise HTTPException(HTTPStatus.BAD_REQUEST, str(e))
            else:
                duration = time.time() - start_time
                self.logger.error(f"Transaction creation failed - From: {request.address_from}, To: {request.address_to}, Asset: {request.asset}, Error: {str(e)}, Duration: {duration}s")
                record_transaction_created(request.asset, "error")
                raise
        except Exception as e:
            duration = time.time() - start_time
