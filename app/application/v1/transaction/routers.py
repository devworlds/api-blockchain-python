...
from app.application.v1.transaction.usecase import GetTransactionHash, CreateOnChainTransaction, ListTransactions
from app.application.v1.transaction.handlers import check_transaction_handler, create_onchain_transaction_handler, list_transactions_handler
from app.application.v1.transaction.schemas import TransactionHashResponse, TransactionOnChainRequest, TransactionOnChainResponse, TransactionDTO
...
from app.infrastructure.db.wallet.postgresql_repository import HashiCorpVaultService, EthereumWalletService
...
@router.get("/transaction/{tx_hash}", response_model=TransactionHashResponse, tags=["Transaction"])
...
@router.get("/transaction/status/{tx_hash}", response_model=TransactionOnChainResponse, tags=["Transaction"])
async def get_transaction_status(
    ...
    return TransactionOnChainResponse(
        ...
        effective_fee=float(wei_to_eth(tx_data["effective_fee"])) if tx_data["effective_fee"] else 0.0,
...
def get_create_onchain_usecase(request: Request):
    ...
    return CreateOnChainTransaction(web3_repo, transaction_repo, vault_service, wallet_service)

@router.post("/transaction", response_model=TransactionOnChainResponse, tags=["Transaction"])
