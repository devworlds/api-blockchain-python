...
from app.application.v1.wallet.handlers import create_wallets_handler, get_all_wallets_handler
from app.application.v1.wallet.schemas import WalletCreationStatusResponse
from app.infrastructure.db.wallet.postgresql_repository import HashiCorpVaultService, EthereumWalletService, PostgreSQLWalletRepository
...
async def create_wallets(
    ...
    try:
        with open("logs/app.log", "a") as f:
            f.write(f"2025-07-01 16:30:00 | INFO | router | 🚀 POST /wallets called with n={n}\n")
            f.flush()
    ...
    try:
        with open("logs/app.log", "a") as f:
            f.write(f"2025-07-01 16:30:01 | INFO | router | ✅ POST /wallets completed: {result.status}\n")
            f.flush()
    except Exception as e:
        pass
