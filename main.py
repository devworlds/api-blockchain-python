...
from app.shared.monitoring.transaction_monitor import TransactionMonitorService, TransactionMonitorManager
import asyncpg
...
async def lifespan(app: FastAPI):
    logger.info(f"Starting API Blockchain Python v{config.app_version} ({config.environment})")
    ...
    try:
        ...
        if app.state.web3.is_connected():
            logger.info(f"Web3 connection established - Provider: {config.web3_provider_url}, Chain ID: {app.state.web3.eth.chain_id}")
        else:
            logger.warning(f"Web3 connection failed - Provider: {config.web3_provider_url}")
...
async def logging_middleware(request: Request, call_next):
    ...
    if request.url.path in ["/health", "/metrics"]:
        logger.debug(f"HTTP {request.method} {request.url} from {client_ip} ({user_agent})")
    else:
        logger.info(f"HTTP {request.method} {request.url} from {client_ip} ({user_agent})")
    ...
    try:
        ...
        if request.url.path in ["/health", "/metrics"]:
            logger.debug(f"HTTP {request.method} {request.url} completed - Status: {response.status_code}, Duration: {duration:.3f}s")
        else:
            logger.info(f"HTTP {request.method} {request.url} completed - Status: {response.status_code}, Duration: {duration:.3f}s")
        ...
        # Log error
        logger.error(f"HTTP {request.method} {request.url} failed after {duration:.3f}s: {str(e)}")
...
async def health_check():
    ...
    if hasattr(app.state, 'pool') and app.state.pool:
        try:
            ...
            health_info["database_pool_used"] = app.state.pool.get_size() - app.state.pool.get_idle_size()
            
            # Update metrics
            database_connection_pool_size.set(app.state.pool.get_size())
            database_connection_pool_used.set(app.state.pool.get_size() - app.state.pool.get_idle_size())
    ...
    if hasattr(app.state, 'transaction_monitor_manager'):
        try:
            health_info["transaction_monitors"] = await app.state.transaction_monitor_manager.health_check()
        except Exception as e:
...
def test_logs():
    ...
    file_logger.add("logs/app.log", level="INFO", 
    ...
    logger.info("Structured log test", 
                ...
                user_id="test_user",
                duration=0.123)
    file_logger.info("Structured log test - FILE LOG", 
                     extra={"component": "test", "operation": "log_generation", "user_id": "test_user"})
